...
void print_chrom(Chrom chrom, char * title)
{
    ...
    fprintf(myout, "\n=====chromosome[objv,fitv,ufitv,iter,bestT] %lld %lld %lld %lld %lf =====\n", chrom.ev[EV_OBJV], chrom.ev[EV_FITV], chrom.ev[EV_UFITV], iterations, (gastat.bestT-gastat.startT));
...
{
#ifdef PGAMODE
    fprintf(myout, "=====stat[totalImpv,migImpv,T,bestT,min,max,avg] %d %d %lf %lf %lld %lld %lld\n", gastat.total_improve, gastat.mig_improve, gastat.bestT - gastat.startT, gastat.endT - gastat.startT, gastat.min_fitv, gastat.max_fitv, gastat.avg_fitv);
#else
...
void emigrate(int newimprove)
{
    ...
    if (newimprove >= 5) {
        ...
        // no improvements, randomly pick two w/ binary tour
        n_elite = 0; n_rand = 2;
    }
    // fill emi_buffer
    ...
    for (i=0; i<n_rand; i++) {
        binary_tournament(&p1, &p2);
        *emi_index = MIG_RANDOM; emi_index++;
        for (j=0; j<n; j++) {
...
int immigrate(Chrom * imi)
{
    // search the pool for an unprocessed immigrant
    //TODO: the impl is quite naive. inefficient when the pool size is big
    int i, j, type;
    int * imi_index = imi_buffer;
    for (i=0; i<neighbor_count * emi_size; i++) {
        if (*imi_index) {
            type = *imi_index;
            for (j=0; j<n; j++) {
                imi->solution[j] = *(imi_index+j+1);
            }
            *imi_index = 0; // mark as processed
            return type;
        }
        imi_index += 1+n;
    }
...
void * search(void * args) 
{
    ...
    unsigned long long last_mig_iter = 0;
    int imiType = 0;
#endif
    ...
    if (debug) 
        ...
        seed1 = (int) (get_ga_time() + getpid());
#ifdef GSL_SPRNG
#ifdef PGAMODE
    int myrank, net_size;
    MPI_Comm_rank( MPI_COMM_WORLD, &myrank );
    MPI_Comm_size( MPI_COMM_WORLD, &net_size);
    gsl_sprng_r = gsl_sprng_alloc(myrank, net_size, seed1, NULL);
#else
    gsl_sprng_r = gsl_sprng_alloc(0, 1, seed1, NULL);
#endif
#else
    srand48(seed1); 
#endif
...
//        print_rank(EV_COUNT);
    ...
    do
    {
        ...
        if (iterations - last_mig_iter >= migrate_freq) {
...
#else
            // emigrate
            emigrate(new_improve);
            fprintf(myout, "+++emi: on %d new improves\n", new_improve);
...
//#else
            ...
            MPI_Barrier(migComm);
            MPI_Gather(emi_buffer, emi_size*(1+n), MPI_INT, imi_buffer, emi_size * (1+n), MPI_INT, myrank, migComm);
            MPI_Barrier(migComm);
            if (debug) print_imi();
//#endif
#endif
            new_improve = 0; // reset
            last_mig_iter = iterations;
        }
#endif
    // GA: stopping rules 
    } while (/*(iterations<max_iterations) && (converged==0) && */(no_improve < improve_threshold) || (stopping_quality>0 && elite_chrom.ev[EV_FITV]<=stopping_quality && elite_chrom.ev[EV_UFITV]==0)/*&& (debug==0)*/);
        
    gastat.endT = get_ga_time();
    
    // output results (elite) 
    print_chrom(elite_chrom, "The Best Solution");
...
int main(int argc, char ** argv)
{
    ...
    int myrank;
    MPI_Comm_rank(MPI_COMM_WORLD, &myrank);
    sprintf(outputpath, "%s/%s.%d.myout", myoutputdir, glogPrefix, myrank);
#else
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    switch (argv[i][1]) {
		    ...
		    exit (0);
                ...
                i++;
                int number;
                if (i >= argc || (sscanf (argv[i], "%d", &number) != 1)) {
		    printf ("\"%s -?\" for help.\n", argv[0]);
		    exit (0);
	        }
                if (strategy_stop == 'f')
...
*/
	        ...
	        printf ("Usage: %s [-s r|b] [-r u|n] [-i n|c|r] [-p pop_size] [-t i|f number] [-f data] [-d] [-e] [-B] [-S] [-M freq rate] \n", argv[0]);
/* >>> */
	    ...
	    printf ("      -q : specify soln quality as stopping rule. PGA only\n");
...
*/
	    ...
	    printf ("      -M : specify migration freq and rate. PGA only\n");
